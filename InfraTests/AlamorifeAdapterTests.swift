...
class AlamorifeAdapter: HttpPostClient {
...
class AlamorifeAdapterTests: XCTestCase {
    ...
    func test_post_should_make_request_with_error_when_request_completes_with_error() {
        expectResult(.failure(.noConnectivity), when: (data: nil, response: nil, error: makeError()))
    }
    
    func test_post_should_make_request_with_error_on_all_invalid_cases() {
        expectResult(.failure(.noConnectivity), when: (data: makeValidData(), response: makeHttpResponse(statusCode: 200), error: makeError()))
